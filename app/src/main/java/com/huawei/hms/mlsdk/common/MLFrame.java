...
public class MLFrame {
            ...
            if (byteBuffer != null) {
                if (byteBuffer.capacity() >= i10 * i11) {
                    if (i12 != 17 && i12 != 16 && i12 != 842094169) {
                        throw new IllegalArgumentException(i.a("Parameter formatType:", i12, " is not supported"));
                    }
    ...
    public static MLFrame fromFilePath(Context context, Uri uri) {
        if (context != null) {
            if (uri != null) {
    ...
    public static MLFrame fromMediaImage(Image image, int i10) {
        ...
        if (format != 256 && format != 35) {
            StringBuilder a10 = d.a("Unsupported format: ");
            a10.append(image.getFormat());
            a10.append(", Only JPEG and YUV_420_888 are supported");
        ...
        if (format == 256) {
            ...
            if (planes != null && planes.length == 1 && (plane = planes[0]) != null && plane.getBuffer() != null) {
                ...
                buffer.get(bArr);
                mLFrame = i10 != 0 ? new MLFrame(rotate(BitmapFactory.decodeByteArray(bArr, 0, remaining), i10)) : new MLFrame(bArr);
            }
        } else {
    ...
    private Pair<Integer, Integer> getPreviewSize() {
        ...
        if (property != null) {
            if (property.getItemIdentity() != -1) {
                ...
                valueOf = Integer.valueOf(z10 ? property2.getHeight() : property2.getWidth());
                height = z10 ? this.property.getWidth() : this.property.getHeight();
            } else {
                valueOf = Integer.valueOf(wrapBitmap().getWidth());
                height = wrapBitmap().getHeight();
            }
            return Pair.create(valueOf, Integer.valueOf(height));
        }
        return null;
    }
